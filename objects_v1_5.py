...
def get_object(post_quantity=10):
    ...
    for name, obj in objects['top']['children'].items():
        ...
        obj['children']['valve_foam']['do_open'] = f'do_{module_number}_{(post_number - 1) % 3 * 7 + 1}'
        obj['children']['valve_wax']['do_open'] = f'do_{module_number}_{(post_number - 1) % 3 * 7 + 2}'
        obj['children']['valve_shampoo']['do_open'] = f'do_{module_number}_{(post_number - 1) % 3 * 7 + 3}'
        obj['children']['valve_cold_water']['do_open'] = f'do_{module_number}_{(post_number - 1) % 3 * 7 + 4}'
        obj['children']['valve_hot_water']['do_open'] = f'do_{module_number}_{(post_number - 1) % 3 * 7 + 5}'
        obj['children']['valve_osmos']['do_open'] = f'do_{module_number}_{(post_number - 1) % 3 * 7 + 6}'
        obj['children']['valve_intensive']['do_open'] = f'do_{module_number}_{(post_number - 1) % 3 * 7 + 7}'
